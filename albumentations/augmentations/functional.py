...
def non_rgb_warning(image):
    if not is_rgb_image(image):
        ...
        if is_multispectral_image(image):
...
def shift_rgb(img, r_shift, g_shift, b_shift):
    if img.dtype == np.uint8:
        ...
        r_shift, g_shift, b_shift = np.int32(
            r_shift), np.int32(g_shift), np.int32(b_shift)
    else:
        # Make a copy of the input image since we don't want to modify it directly
        img = img.copy()
    img[..., 0] += r_shift
...
def pad(img, min_height, min_width, border_mode=cv2.BORDER_REFLECT_101, value=None):
    ...
    img = pad_with_params(img, h_pad_top, h_pad_bottom,
                          w_pad_left, w_pad_right, border_mode, value)
...
def pad_with_params(img, h_pad_top, h_pad_bottom, w_pad_left, w_pad_right, border_mode=cv2.BORDER_REFLECT_101,
                    value=None):
    img = cv2.copyMakeBorder(img, h_pad_top, h_pad_bottom,
                             w_pad_left, w_pad_right, border_mode, value=value)
    return img
...
def jpeg_compression(img, quality):
    ...
    if input_dtype == np.float32:
        ...
        raise ValueError(
            'Unexpected dtype {} for Jpeg augmentation'.format(input_dtype))
...
def add_rain(img, slant, drop_length, drop_width, drop_color, blur_value, brightness_coefficient, rain_drops):
    ...
    for (rain_drop_x0, rain_drop_y0) in rain_drops:
        ...
        cv2.line(image, (rain_drop_x0, rain_drop_y0),
                 (rain_drop_x1, rain_drop_y1), drop_color, drop_width)
...
def add_shadow(img, vertices_list):
    ...
    image_hls[:, :, 1][red_max_value_ind] = image_hls[:,
                                                      :, 1][red_max_value_ind] * 0.5
...
def bbox_pad(bbox, height, width, h_pad_top, h_pad_bottom, w_pad_left, w_pad_right,
    ...
    if border_mode == cv2.BORDER_CONSTANT:
        ...
        if h_padded_size <= 0 or w_padded_size <= 0:
            raise ValueError(
                'Invalid padding shape ({},{})'.format(
                    h_padded_size, w_padded_size)
            )
        return [
...
def bbox_flip(bbox, d, rows, cols):
    ...
    if d == 0:
        ...
        raise ValueError(
            'Invalid d value {}. Valid values are -1, 0 and 1'.format(d))
    return bbox
...
def keypoint_rotate(keypoint, angle, rows, cols, **params):
    matrix = cv2.getRotationMatrix2D(
        ((cols - 1) * 0.5, (rows - 1) * 0.5), angle, 1.0)
    x, y, a, s = keypoint
    x, y = cv2.transform(np.array([[[x, y]]]), matrix).squeeze()
