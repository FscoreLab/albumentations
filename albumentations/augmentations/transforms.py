...
class PadIfNeeded(DualTransform):
    ...
    def apply_to_bbox(self, bbox, pad_top=0, pad_bottom=0, pad_left=0, pad_right=0, rows=0, cols=0, **params):
        ...
        bbox = [x_min + pad_left, y_min + pad_top,
                x_max + pad_left, y_max + pad_top]
...
class RandomSizedCrop(DualTransform):
    ...
    def get_params(self):
        crop_height = random.randint(
            self.min_max_height[0], self.min_max_height[1])
        return {'h_start': random.random(),
                'w_start': random.random(),
...
class RandomSizedBBoxSafeCrop(DualTransform):
    ...
    def get_params_dependent_on_targets(self, params):
        ...
        bx2, by2 = x2 + (1 - x2) * random.random(), y2 + \
            (1 - y2) * random.random()
        bw, bh = bx2 - bx, by2 - by
...
class ElasticTransform(DualTransform):
    ...
    def apply(self, img, random_state=None, interpolation=cv2.INTER_LINEAR, **params):
        return F.elastic_transform(img, self.alpha, self.sigma, self.alpha_affine, interpolation,
                                   self.border_mode, self.value, np.random.RandomState(
                                       random_state),
                                   self.approximate)
    ...
    def apply_to_mask(self, img, random_state=None, **params):
        return F.elastic_transform(img, self.alpha, self.sigma, self.alpha_affine, cv2.INTER_NEAREST,
                                   self.border_mode, self.mask_value, np.random.RandomState(
                                       random_state),
                                   self.approximate)
...
class RandomSunFlare(ImageOnlyTransform):
    ...
    def get_params_dependent_on_targets(self, params):
        ...
        for i in range(num_circles):
            ...
            r_color = random.randint(
                max(self.src_color[0] - 50, 0), self.src_color[0])
            g_color = random.randint(
                max(self.src_color[0] - 50, 0), self.src_color[0])
            b_color = random.randint(
                max(self.src_color[0] - 50, 0), self.src_color[0])
...
class RandomShadow(ImageOnlyTransform):
    ...
    def get_params_dependent_on_targets(self, params):
        ...
        num_shadows = random.randint(
            self.num_shadows_lower, self.num_shadows_upper)
        ...
        for index in range(num_shadows):
            ...
            for dimensions in range(self.shadow_dimension):
                vertex.append((random.randint(x_min, x_max),
...
class RandomContrast(RandomBrightnessContrast):
    def __init__(self, limit=0.2, always_apply=False, p=0.5):
        super(RandomContrast, self).__init__(brightness_limit=0,
                                             contrast_limit=limit, always_apply=always_apply, p=p)
        warnings.warn(
            "This class has been deprecated. Please use RandomBrightnessContrast", DeprecationWarning)
...
class ChannelDropout(ImageOnlyTransform):
    ...
    def get_params_dependent_on_targets(self, params):
        ...
        channels_to_drop = random.sample(
